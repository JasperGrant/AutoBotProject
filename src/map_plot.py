...
map_file = open("data/map10.csv", "r").read()
...
robot_pose = [
    points.split(",")
    for points in open("data/pose.csv", "r").read().split("\n")
    if points
]
...
plt.ylim(0, 12)
plt.xlabel("X")
# Muliply x axis marks by 15 to get cm
plt.xticks([i for i in range(0, 13)], [i * 15 for i in range(0, 13)])
plt.yticks([i for i in range(0, 13)], [i * 15 for i in range(0, 13)])
plt.ylabel("Y")
